...
public class PauseableUploadTask {
    ...
    public void upload(String uploadId) throws ClientException, ServiceException, IOException {
        ...
        try {
            ...
            Logger.d("ListPartsFound", String.valueOf(listResult.getParts().size()));
            ...
            while (currentUploadIndex <= totalBlockNum) {
                ...
                currentUploadIndex++;
                Logger.d("UploadPartIndex", String.valueOf(currentUploadIndex - 1));
                Logger.d("UploadPartSize", String.valueOf(currentUploadLength));

                //如果暂停了就直接退出，需要断点续传的话直接使用同样的参数构建任务上传即可
                if (isPause()) {
                    Logger.w("MultiPartUpload", "Pause");
                    Logger.w("UploadId", uploadId);
                    return;
                }
            }
            ...
            setComplete();
            Logger.d("multipartUpload", "multipart upload success! Location: " + completeResult.getLocation());
            ...
            Logger.e("ErrorCode", e.getErrorCode());
            Logger.e("RequestId", e.getRequestId());
            Logger.e("HostId", e.getHostId());
            Logger.e("RawMessage", e.getRawMessage());
