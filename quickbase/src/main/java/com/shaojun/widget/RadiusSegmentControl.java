 ...
 * 圆角分割控制按钮
 */
public class RadiusSegmentControl extends View {
    ...
    private int mHorizonGap;//水平间隙
    private int mVerticalGap;//垂直间隙
    ...
    public RadiusSegmentControl(Context context) {
        this(context, null);
    }

    public RadiusSegmentControl(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public RadiusSegmentControl(Context context, AttributeSet attrs, int defStyle) {
        super(context, attrs, defStyle);

        TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.RadiusSegmentControl);

        String textArray = ta.getString(R.styleable.RadiusSegmentControl_texts);
        if (textArray != null) {
            mTexts = textArray.split("\\|");
        }

        mTextSize = ta.getDimensionPixelSize(R.styleable.RadiusSegmentControl_android_textSize, (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_SP, 14, context.getResources().getDisplayMetrics()));
        mCornerRadius = ta.getDimensionPixelSize(R.styleable.RadiusSegmentControl_cornerRadius, (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 5, context.getResources().getDisplayMetrics()));
        mDirection = Direction.values()[ta.getInt(R.styleable.RadiusSegmentControl_direction, 0)];

        mHorizonGap = ta.getDimensionPixelSize(R.styleable.RadiusSegmentControl_horizonGap, 0);
        mVerticalGap = ta.getDimensionPixelSize(R.styleable.RadiusSegmentControl_verticalGap, 0);

        int gap = ta.getDimensionPixelSize(R.styleable.RadiusSegmentControl_gaps, (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 2, context.getResources().getDisplayMetrics()));
        ...
        mColors = ta.getColorStateList(R.styleable.RadiusSegmentControl_colors);
        mSelectedTextColor = ta.getColor(R.styleable.RadiusSegmentControl_textSelectedColors, Color.WHITE);
        if (mColors == null) {
            mColors = new ColorStateList(new int[][]{{}}, new int[]{0xFF0099CC});
        }

        mBoundWidth = ta.getDimensionPixelSize(R.styleable.RadiusSegmentControl_boundWidth, mBoundWidth);
        mSeparatorWidth = ta.getDimensionPixelSize(R.styleable.RadiusSegmentControl_separatorWidth, mSeparatorWidth);
